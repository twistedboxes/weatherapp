...
async function getData(userInput) {
  ...
  let data = await fetch(
    `https://api.open-meteo.com/v1/forecast?${croppedCoord}&hourly=temperature_2m,relativehumidity_2m,precipitation_probability,weathercode,windspeed_10m&windspeed_unit=ms&timezone=auto`
  );
  ...
  wmo.innerText = switcher(wmolist);
  ...
  precipitationProbability.innerText =
    dataJson.hourly.precipitation_probability[croppedtime] + "%";
  switcher(wmolist);
